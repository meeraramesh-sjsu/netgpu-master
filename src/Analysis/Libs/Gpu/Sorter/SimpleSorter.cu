...
#define MAX_BUFFER_PACKETS 3840
...
//		result[i] = i&0x7326432;
	...
	wrapper_SimpleSort<float,float,float>((float*)a,0,MAM-1,(float*)b);
...
int main(int args, char *argv[]){
        int i;
	int counter;	
	uint64_t *a,*b,*result,*result2;
	cudaMalloc((void **)&a,sizeof(uint64_t)*MAX_BUFFER_PACKETS);
        cudaMalloc((void **)&b,sizeof(uint64_t)*MAX_BUFFER_PACKETS);
	result = (uint64_t*)malloc(sizeof(uint64_t)*MAX_BUFFER_PACKETS);
	result2 = (uint64_t*)malloc(sizeof(uint64_t)*MAX_BUFFER_PACKETS);
...
//		result[i] = i&0x7326432;
	...
	cudaMemcpy(a,result,sizeof(uint64_t)*MAX_BUFFER_PACKETS, cudaMemcpyHostToDevice);
	cudaThreadSynchronize();       

	cudaMemcpy(b,result2,sizeof(uint64_t)*MAX_BUFFER_PACKETS, cudaMemcpyHostToDevice);
	cudaThreadSynchronize();  
	unsigned int t1,t2;	
	t1=clock();

	wrapper_SimpleSort(a,0,MAM-1,b,(uint32_t*)NULL);

	t2=clock();
	fprintf(stderr,"Temps aproximat: %d\n",t2-t1);
	cudaMemcpy(result,a,sizeof(uint64_t)*MAX_BUFFER_PACKETS, cudaMemcpyDeviceToHost);
	cudaThreadSynchronize();
